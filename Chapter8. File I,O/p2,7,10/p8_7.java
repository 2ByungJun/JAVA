...
public class p8_7 {
	...
	public static void main(String[] args) {
		...
		File f1 = new File("c:\\temp\\a.jpg");
		File f2 = new File("c:\\temp\\b.jpg");
		try {
			srcStream = new BufferedInputStream(new FileInputStream(f1)); // 버퍼 입력 스트림에 연결
			destStream = new BufferedOutputStream(new FileOutputStream(f2)); // 버퍼 출력 스트림에 연결
			
			long tenPercent = f1.length() / 10; // 파일의 10% 크기
